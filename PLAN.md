...
- Screenshot Service: снимает все дисплеи с минимальным читаемым разрешением (таргет уточнить), сжимает и сохраняет с таймстампами/ID кванта.
  - Update 2025-12-16: в MVP сохраняем по одному файлу на дисплей (не “сшиваем” в композит), базовая директория — Application Support (внутри папки приложения).
...
- Схема данных (SQLite, зашифровано):
  ...
  - Quantum(id, session_id, start_at, end_at, is_partial, partial_reason(drop|too_short|none), actual_duration_seconds, activity_percent, is_active_flag, primary_app, primary_category, switches_count, focus_mode_flag, anomaly_switching_flag, primary_screenshot_id)
    - Update 2025-12-16: `screenshot_id` → `primary_screenshot_id` (указатель на “превью/primary” скриншот, если на квант есть несколько файлов по дисплеям).
...
- ~~Как представляем multi-display скриншоты в модели данных: несколько `ScreenshotMetadata` на один `quantum_id`; что делать с `Quantum.screenshot_id` (оставить как “primary/preview” или убрать в будущей миграции)? (2025-12-16)~~ Resolved 2025-12-16: оставляем `Quantum.primary_screenshot_id` как быстрый указатель на “primary/preview” скриншот; остальные дисплеи — отдельные `ScreenshotMetadata` с тем же `quantum_id`.
